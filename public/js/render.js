window.Render = (function () {
  ...
  function ensureStars(w, h) {
    ...
    stars = { w, h, pts: [] };
    const count = Math.min(220, Math.round(120 * (w / 1200)));
  ...
  function drawWind(ctx, wind, w, windEnabled) {
    const cx = w / 2;
    const cy = 36;
    ctx.save();
  ...
  function trailColor(weapon) {
    switch (weapon) {
      case 'nuke': return '#ff6644';
      case 'dirt':
      case 'megadirt': return '#c4a882';
      case 'bounce': return '#b8ff3c';
      case 'digger': return '#aa7744';
      case 'napalm': return '#ff6622';
      case 'mirv': return '#ff2d95';
      default: return '#7cf0ff';
    }
  }

  function drawTank(ctx, tank, isActive, showAim, wind, bombSpeed) {
    ...
    if (showAim && isActive) {
      ...
      const speedMul = (P().bombSpeedScale && P().bombSpeedScale(bombSpeed)) || 1;
      let vx = Math.cos(tank.angle) * ((tank.power / 100) * 14 + 2) * speedMul;
      let vy = Math.sin(tank.angle) * ((tank.power / 100) * 14 + 2) * speedMul;
      const wforce = (wind || 0) * 0.012;
      ctx.moveTo(x, y);
      for (let i = 0; i < 40; i++) {
        vx += wforce;
        vy += P().GRAVITY;
  ...
  function drawProjectile(ctx, proj) {
    if (!proj) return;
    ctx.save();
    const col = trailColor(proj.weapon);
  ...
  function drawHUD(ctx, state, opts) {
    ...
    roundRect(ctx, 12, h - 78, 300, 64, 12);
    ctx.fill();
    ctx.fillStyle = '#9ecbff';
    ...
    if (opts && opts.showHelp) {
      ctx.fillStyle = 'rgba(0,0,0,0.35)';
      roundRect(ctx, w - 250, h - 78, 238, 64, 12);
      ctx.fill();
      ctx.fillStyle = 'rgba(255,255,255,0.7)';
      ctx.font = '10px Segoe UI, sans-serif';
      ctx.textAlign = 'left';
      ctx.fillText('Pad / keys: aim · power · move', w - 238, h - 52);
      ctx.fillText('Drag arena to aim · FIRE / Space', w - 238, h - 34);
      ctx.fillText('Weapons · Menu / Esc', w - 238, h - 16);
    }
    ctx.restore();
  }
  ...
  function frame(ctx, state, opts = {}) {
    ...
    drawWind(ctx, state.wind || 0, w, state.windEnabled);
    for (const tank of state.tanks) {
      const active = tank.id === state.currentTurn && state.phase === 'aiming';
      const localControl = opts.canControl !== false;
      drawTank(ctx, tank, active, active && localControl && !tank.isAI, state.wind, state.bombSpeed);
    }
    const projs = state.projectiles && state.projectiles.length
      ? state.projectiles
      : state.projectile
        ? [state.projectile]
        : [];
    for (const p of projs) drawProjectile(ctx, p);
